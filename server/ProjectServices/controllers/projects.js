...
module.exports = {
  ...
  getAssignProject: async (req, res) => {
    const { project_id } = req.params;
    if (!project_id)
      return res.status(400).send({ message: "project_id is required" });
    let { recordset } = await db.exec("sp_getAssignedProject", {
      project_id,
    });
    res.send({ User: recordset });
  },
  assignProject: async (req, res) => {
    const { error } = validateAssignProject(req.body);
    ...
    try {
      ...
      checkassign = await db.exec("sp_checkassignedProject", { user_id });

      if (checkassign.recordset.length > 0) {
        let data = checkassign.recordset[0].project_id;
        return res.status(404).send(`user already assigned ${data}`);
      }
      const id = uuidv4();
      await db.exec("sp_assignProject", { id, project_id, user_id });

      res.send({ message: "User added to project successfully" });
    } catch (error) {
  ...
  unassignProject: async (req, res) => {
    try {
      ...
      await db.exec("sp_unassignProject", { project_id });

      res.send({ message: "User unassigned to project successfully" });
    } catch (error) {
      console.log(error.message);
      res
        .status(500)
